...
import { requestSaleAction } from "@/app/actions/sales";
...
export function RequestSaleForm({ eligibility }: { eligibility: PurchaseEligibility }) {
  ...
  const exceedsEligibility = Number(cost) > eligibility.availableCapacity;
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await requestSaleAction(v);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Your savings can accommodate up to <strong>{naira(eligibility.availableCapacity)}</strong> more in credit
        purchases. Your request goes to the Treasurer/Admin for review.
      </p>
      <Card>
        <CardHeader>
