...
import { importMembersAction } from "@/app/actions/memberImport";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Table, THead, TBody, TR, TH, TD } from "@/components/ui/table";

export function MemberImportForm() {
  const [state, action, pending] = useActionState(importMembersAction, undefined);
      ...
      {state?.result && (
        <Card>
          <CardContent className="space-y-3 p-4">
            <p className="text-sm">
              {state.result.total} row(s) processed — <strong>{state.result.applied} created</strong>,{" "}
