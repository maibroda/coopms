...
import { requestLoanAction } from "@/app/actions/loans";
...
export function RequestLoanForm({ eligibility, loanProducts = [] }: { eligibility: EligibilityResult; loanProducts?: LoanProductOption[] }) {
  ...
  const exceedsEligibility = Number(amount) > eligibility.availableToBorrow;
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await requestLoanAction(v);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <p className="text-sm text-muted-foreground">
        You&apos;re eligible to request up to <strong>{naira(eligibility.availableToBorrow)}</strong> (based on your
        savings, less anything already borrowed or pending). Your request goes to the Treasurer/Admin for review.
      </p>
      <Card>
        <CardHeader>
